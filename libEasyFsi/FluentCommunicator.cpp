...
namespace EasyLib {
    ...
    void FluentCommunicator::create(int myid, int np, func_MPT_csend* csend, func_MPT_crecv* crecv)
    {
        myid_ = myid;
        np_ = np;
        fsend_ = csend;
    ...
    {
        if      (_stricmp(name, "myid") == 0)myid_ = value;
        else if (strcmp(name, "MPT_CHAR") == 0)mpt_char_type_ = value;
        else if (strcmp(name, "MPT_SHORT") == 0)mpt_short_type_ = value;
        else if (strcmp(name, "MPT_INT") == 0)mpt_int_type_ = value;
        else if (strcmp(name, "MPT_LONG") == 0)mpt_long_type_ = value;
        else if (strcmp(name, "MPT_FLOAT") == 0)mpt_float_type_ = value;
        else if (strcmp(name, "MPT_DOUBLE") == 0)mpt_double_type_ = value;
        else if (strcmp(name, "MPT_LONG_LONG") == 0)mpt_long_long_type_ = value;
        else {
