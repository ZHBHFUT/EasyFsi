...
namespace EasyLib {
    ...
    {
        if (::strcmp(name, "MPI_STATUSES_IGNORE") == 0)MPI_STATUSES_IGNORE_ = value;
        else {
            error("unsupported MPI constant \"%s\", FILE = %s, FUNC = %s, LINE = %s.", name, __FILE__, __FUNCTION__, __LINE__);
        }
    }
    void MPICommunicator::set_function(const char* name, void* value)
    {
        ...
        else if (::strcmp(name, "MPI_Isend") == 0)MPI_Isend_ = (func_MPI_Isend)value;
        //else if (::strcmp(name, "MPI_Irecv") == 0)MPI_Irecv_ = (func_MPI_Irecv)value;
        else if (::strcmp(name, "MPI_Waitall") == 0)MPI_Waitall_ = (func_MPI_Waitall)value;
        else {
    ...
    void MPICommunicator::async_send(const void* data, int count, DataType type, int dest_rank, int tag)
    {
        if (!MPI_Isend_) {
            error("MPI_Isend is not specified! FILE = %s, FUNC = %s, LINE = %d", __FILE__, __FUNCTION__, __LINE__);
        }
        else {
            requests_.push_back(0);
            MPI_Isend_(data, count, dt2mpt_(type), dest_rank, tag, comm_, &requests_.back());
        }
    }
    void MPICommunicator::wait()
    {
        if (requests_.empty())return;
        if (!MPI_Waitall_)
            error("MPI_Waitall is not specified! FILE = %s, FUNC = %s, LINE = %d", __FILE__, __FUNCTION__, __LINE__);
        else
            MPI_Waitall_(static_cast<int>(requests_.size()), requests_.data(), MPI_STATUSES_IGNORE_);
        requests_.clear();
    }
}
